...
pip install -r python/requirements.txt   # ← 반드시 루트에서 실행
deactivate
...
```bash
# macOS .app 번들만 (dist/mac/ 또는 dist/mac-arm64/ 폴더에 생성)
npm run build:mac:app

# macOS DMG + .app 동시 빌드 (x64 + arm64)
npm run build:mac
...
> **참고**: `.app` 빌드 전에 `assets/icon.icns`가 필요합니다.  
> 없다면 먼저 `./scripts/build-icns.sh`를 실행하세요 (macOS 전용).
